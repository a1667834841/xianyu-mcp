...
class StableSearchRunner:
    ...
    async def search(self, params: SearchParams):
        ...
        while len(all_items) < params.rows:
            ...
            print(
                f"[PageApi] 第 {page_number} 页解析完成 raw_items={len(result.items)} new_items={new_count} total_items={len(all_items)}",
                flush=True,
            )

            if len(all_items) >= params.rows:
                print(
                    f"[PageApi] 停止：达到目标 rows={params.rows} pages_fetched={page_number}",
                    flush=True,
                )
            ...
            if stale_pages >= self.max_stale_pages:
                print(
                    f"[PageApi] 停止：连续无新增 stale_pages={stale_pages} pages_fetched={page_number} total_items={len(all_items)}",
                    flush=True,
                )
                return SearchOutcome(
                    items=all_items[: params.rows],
