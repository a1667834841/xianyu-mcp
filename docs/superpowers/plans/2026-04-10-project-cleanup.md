...
- 修改: `tests/test_page_isolation.py` (删除依赖废弃代码的测试)
...
- [ ] **Step 5: 更新 test_page_isolation.py**

删除依赖 `_BrowserSearchImpl` 和 `_build_page_api_runner` 的测试函数：
- 第372-440行区域的测试函数
- 第480-574行区域的测试函数
- 第581-703行区域的测试函数
- 第706-776行区域的测试函数

**保留**: 锁机制和页面隔离相关测试（如 `test_xianyu_app_uses_distinct_role_locks` 等）
...
## Task 6: 清理 core.py 死代码链

**Files:**
- 修改: `src/core.py` (删除 `_build_page_api_runner`、`get_search_detail`、`_BrowserSearchImpl`)

- [ ] **Step 1: 确认代码位置**

```bash
grep -n "_build_page_api_runner\|get_search_detail\|_BrowserSearchImpl" src/core.py
```

预期输出：
- 第308行: `get_search_detail()` 方法
- 第367行: `_build_page_api_runner()` 函数
- 第388行: `_BrowserSearchImpl` 类

- [ ] **Step 2: 删除 get_search_detail 方法**

删除整个 `get_search_detail()` 方法定义（约第308-380行）。

- [ ] **Step 3: 删除 _build_page_api_runner 函数**

删除整个 `_build_page_api_runner()` 函数定义。

- [ ] **Step 4: 删除 _BrowserSearchImpl 类**

删除整个 `_BrowserSearchImpl` 类定义（约第388-500行）。
...
```bash
git add src/core.py
git commit -m "refactor(core): 删除废弃的 get_search_detail、_build_page_api_runner、_BrowserSearchImpl"
```
